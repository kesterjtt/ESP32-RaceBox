...
//plan to pull G Force YXZ and speed to log in a MAXXECU of CAN
// CAN settings
static const uint32_t CAN_BIT_RATE = 500000UL;  // 500 kbps
static const uint32_t CAN_ID = 0x600;
...
void setup() {
  ...
  // CAN Initialization
  ACAN_ESP32_Settings canSettings(CAN_BIT_RATE);
  canSettings.mTxPin = CAN_TX_PIN;
  canSettings.mRxPin = CAN_RX_PIN;
  const uint32_t errorCode = ACAN_ESP32::can.begin(canSettings);
  if (errorCode == 0) {
...
void loop() {
  ...
  if (connected) {  //if we are connected to RaceBox
    interpret_serial_input();  // Listen to serial console inputs
    if (updated_RaceBox_Data_Message) {  // If we have received updated values from a RaceBox data message
      print_RaceBox_Data_message_payload_to_oled();
      print_RaceBox_Data_message_payload_to_serial();
      updated_RaceBox_Data_Message = false;  // Reset the flag
    }

  
    // Combine speed and time into one CAN message
    CANMessage frame;
    uint16_t canSpeed = (speed * 3.6 / 1000);  // Speed in km/h
    uint8_t canSecond = second;                // Time in seconds

    frame.id = CAN_ID;  // CAN ID 0x600
    frame.len = 3;      // Message length of 3 bytes

    frame.data[0] = canSpeed >> 8;             // High byte of speed
    frame.data[1] = canSpeed & 0xFF;           // Low byte of speed
    frame.data[2] = canSecond;                 // Time in seconds

    
    bool ok = ACAN_ESP32::can.tryToSend(frame);

    if (ok) {
      Serial.println("Speed and time data sent over CAN.");
    } else {
      Serial.println("Failed to send speed and time data over CAN.");
    }

// Send G-Force X, Y, and Z data over CAN with ID 0x601
    CANMessage gForceFrame;
    gForceFrame.id = 0x601;  // CAN ID for G-Force data
    gForceFrame.len = 6;     // Message length of 6 bytes (2 bytes each for X, Y, Z)

    gForceFrame.data[0] = gForceX >> 8;  // High byte of G-Force X
    gForceFrame.data[1] = gForceX & 0xFF;  // Low byte of G-Force X
    gForceFrame.data[2] = gForceY >> 8;  // High byte of G-Force Y
    gForceFrame.data[3] = gForceY & 0xFF;  // Low byte of G-Force Y
    gForceFrame.data[4] = gForceZ >> 8;  // High byte of G-Force Z
    gForceFrame.data[5] = gForceZ & 0xFF;  // Low byte of G-Force Z

    ok = ACAN_ESP32::can.tryToSend(gForceFrame);
    if (ok) {
      Serial.println("G-Force data sent over CAN.");

    } else {
      Serial.println("Failed to send G-Force data over CAN.");
    }


  } else {
    interpret_serial_input();  // Just for debugging purposes if not connected
  }

  // Further operations (e.g., display handling) if any
  if (updated_RaceBox_Data_Message) {
    print_RaceBox_Data_message_payload_to_oled();
    print_RaceBox_Data_message_payload_to_serial();
    updated_RaceBox_Data_Message = false;  // Reset flag
  }
}
